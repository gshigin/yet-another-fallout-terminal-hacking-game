...
class engine
{
  ...
  public:
    ...
    std::size_t get_word_count() const
    {
        return wordCount;
    }
    std::size_t get_word_length() const
    {
        return wordLength;
    }
    std::size_t get_attempts() const
    {
        return attemptsLeft;
    }

    auto get_words() const
    {
        return std::span(words).subspan(0, wordsLeft);
    }
    std::string_view get_term_chars() const
    {
        return {term_chars.begin(), term_chars.end()};
    }
