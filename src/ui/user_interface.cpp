...
user_interface::impl::impl(fcallback_t cb, std::uint64_t seed)
    : callback_(std::move(cb)),
      start_hex_(util::seed(seed)),
      terminal_state_(callback_({input_type::other, {}})),
  ...
  if (terminal_state_.click_res.has_value())  // click happened
  {
    switch (terminal_state_.click_res->state) {
      ...
      case yafth::click_result::entry_denied:
        ...
        log_.push_back(ftxui::text(">" + std::to_string(terminal_state_.click_res->match->of) + "/" + std::to_string(terminal_state_.click_res->match->from) +
                                   " correct."));
        last_loocked_ = "";
        break;
