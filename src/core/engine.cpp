...
engine::engine(const lock_level lock_level_setting_, const uint32_t player_science_skill_, const uint64_t seed_ = 0)
    : rng(0, random::seed(seed_)), lock_level_setting(lock_level_setting_),
      player_science_skill(
          std::clamp<uint32_t>(player_science_skill_, player_science_skill_min, player_science_skill_max)),
      word_length(set_word_length()), word_count(set_word_count()), answer(rng.next() % word_count),
...
core::state engine::process_input(core::input input)
{
    if (input.type == input_type::look && input.coords.has_value())
    {
        auto m_x = input.coords->x;
        auto m_y = input.coords->y;
        ...
        {
            return {term_chars, attempts_left, {}};
        }

        return {term_chars, attempts_left, shared_term.eng.look_at(12 * m_y + m_x)};
    }

    if (input.type == input_type::click && input.coords.has_value())
    {
        auto m_x = input.coords->x;
        auto m_y = input.coords->y;
