...
auto argparser::parse(int argc, char *argv[]) -> args
{
...
auto argparser::get_next_arg_(int argc, char *argv[], int current_index, const std::string &arg_name) -> std::string
{
...
auto argparser::handle_science_(const std::string &value) -> std::uint32_t
{
    int level = 0;
    try
    {
...
auto argparser::handle_lock_(const std::string &value) -> lock_level
{
    static const std::unordered_map<std::string, lock_level> valid_levels = {{"0", lock_level::very_easy}, {"1", lock_level::easy},
                                                                             {"2", lock_level::average},   {"3", lock_level::hard},
                                                                             {"4", lock_level::very_hard}, {"very_easy", lock_level::very_easy},
                                                                             {"easy", lock_level::easy},   {"average", lock_level::average},
                                                                             {"hard", lock_level::hard},   {"very_hard", lock_level::very_hard}};
    if (valid_levels.find(value) == valid_levels.end())
    {
        throw std::invalid_argument("--lock must be a number (0-4) or a string (very_easy, easy, average, hard, very_hard).");
    }
    return valid_levels.find(value)->second;
}

auto argparser::handle_seed_(const std::string &value) -> std::uint64_t
{
    std::uint64_t seed;
    try
    {
...
void argparser::check_defaults_(std::optional<std::uint32_t> &science_level, std::optional<lock_level> &lock, std::optional<std::uint64_t> &seed)
{
    if (!seed.has_value())
    {
        // get current time
    ...
    if (science_level.has_value())
    {
        if (lock.has_value())
        {
            std::uint32_t science_skill_required = *lock == lock_level::very_easy ? 15 : 25 * static_cast<std::uint32_t>(*lock);
            if (!(*science_level >= science_skill_required))
            {
                throw std::invalid_argument("A Science skill of " + std::to_string(science_skill_required) + " is required to hack this terminal. >:(");
            }
        }
        else
        {
